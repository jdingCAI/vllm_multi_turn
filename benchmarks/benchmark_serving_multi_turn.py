...
class RequestArgs(NamedTuple):
    ...
    metrics_url: Optional[str]  # URL for metrics endpoint
...
async def get_kv_cache_metrics(session: aiohttp.ClientSession, metrics_url: str) -> Tuple[Optional[float], Optional[float]]:
    """Fetch KV cache metrics from vLLM server.
    
    Returns:
        Tuple of (queries, hits) or (None, None) if metrics unavailable
    """
    try:
