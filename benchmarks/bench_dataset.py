...
def parse_input_json_file(conf: dict) -> GenConvArgs:
    ...
    # NOTE: "num_turns" in the config means actual conversation turns
    # 1 turn = 1 user message + 1 assistant response (2 messages)
    # So if num_turns=10, you get 20 messages (10 user + 10 assistant)
...
def generate_conversations(
    ...
    # Various prompts for user messages to add diversity
    user_prompts = [
        "Please analyze the following text: ",
        "Can you explain this passage: ",
        "What are your thoughts on: ",
        "Please summarize and expand on: ",
        "Could you provide insights about: ",
        "Help me understand this text: ",
        "Please interpret the following: ",
        "What does this text mean: ",
        "Can you elaborate on: ",
        "Please discuss this content: ",
    ]
    
    # We'll use a default prompt for token counting
    base_prompt_text = user_prompts[0]
    ...
    # A turn = 1 user message + 1 assistant message (2 messages total)
    turn_count: np.ndarray = args.input_num_turns.sample(args.num_conversations)
    
    # Turn count should be at least 1 (one complete exchange)
    turn_count = np.maximum(turn_count, 1)
    
    # Convert turns to messages (each turn = 2 messages)
    ...
    for conv_id in range(args.num_conversations):
        ...
        n_messages = message_count[conv_id]

        # User prompt token count per message (with lower limit)
        input_token_count: np.ndarray = args.input_num_tokens.sample(n_messages)
        input_token_count = np.maximum(input_token_count, base_prompt_token_count)

        # Assistant answer token count per message (with lower limit)
        output_token_count: np.ndarray = args.output_num_tokens.sample(n_messages)
        ...
        for msg_id in range(n_messages):
            if user_turn:
                role = "user"
                num_tokens = input_token_count[msg_id]

                # Generate the user prompt with more variation
                content = ""
                
                # For first turn only, add common prefix if specified
                if len(common_prefix_text) > 0 and msg_id == 0:
                    content = common_prefix_text + " "
                ...
                # Select a varied prompt based on conversation and message
                prompt_idx = (conv_id + msg_id) % len(user_prompts)
                selected_prompt = user_prompts[prompt_idx]
                content += selected_prompt
                num_tokens -= len(tokenizer.encode(selected_prompt, add_special_tokens=False))

                if num_tokens > 0:
                    # Add text from the input file (to reach the desired token count)
                    start_offset = base_offset + msg_id * input_token_count.max()
                ...
                num_tokens = output_token_count[msg_id]
                
                # Use different offsets for assistant responses to create variation
                # Create a unique hash for each conversation-message combination
                # This ensures each assistant response uses a different part of the text
                hash_value = hash(f"assistant_{conv_id}_{msg_id}_{seed}") 
                assistant_offset = abs(hash_value) % (len(list_of_tokens) - num_tokens)
                
                assert len(list_of_tokens) > assistant_offset + num_tokens, (
                    f"Not enough input text to generate {num_tokens} "
                    "tokens for assistant content"
                )
                content = tokenizer.decode(list_of_tokens[assistant_offset:assistant_offset + num_tokens])
